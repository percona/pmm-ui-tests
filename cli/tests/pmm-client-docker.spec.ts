...
test.describe('PMM Client Docker CLI tests', async () => {
  test.beforeAll(async ({}) => {
    ...
    await cli.exec('docker network connect docker-client-check pmm-server')
    await cli.exec('sleep 2');
    await cli.exec('docker exec pmm-client-1 pmm-admin add mysql --username=pmm --password=pmm-pass --service-name=ps-8.0 --query-source=perfschema --host=ps-1 --port=3306 --server-url=https://admin:admin@pmm-server:8443 --server-insecure-tls=true');
    ...
    await cli.exec('docker exec pmm-client-1 pmm-admin add mongodb --username=pmm --password=pmm-pass --service-name=mongodb-7.0  --host=psmdb-1 --port=27017 --server-url=https://admin:admin@pmm-server:8443 --server-insecure-tls=true');
  });
