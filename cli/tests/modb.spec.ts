...
test.describe('Spec file for Percona Server MongoDB (PSMDB) CLI tests ', async () => {
    ...
    test('run pmm-admin add mongodb based on running instances with metrics-mode push', async ({}) => {
        // let ports = (await cli.exec(`pmm-admin list | grep "MongoDB" | awk -F" " '{print $3}'`)).stdout.split('\n');
        let hosts = (await cli.exec(`pmm-admin list | grep "MongoDB" | awk -F" " '{print $3}'`))
            .stdout.trim().split('\n');
        let n = 1;
        for (const host of hosts) {
            let output = await cli.exec(`pmm-admin add mongodb --username=${MONGO_USERNAME} --password=${MONGO_PASSWORD} --metrics-mode=push mongo_inst_${n++} ${host}`);
    ...
    test('run pmm-admin remove mongodb instance added with metrics mode push', async ({}) => {
        let services = (await cli.exec(`pmm-admin list | grep "MongoDB" | grep "mongo_inst_" | awk -F" " '{print $2}'`))
            .stdout.trim().split('\n');
        console.log(services);
        for (const service of services) {
            let output = await cli.exec(`pmm-admin remove mongodb ${service}`);
            await output.assertSuccess();
    ...
    test('run pmm-admin add mongodb based on running instances with metrics-mode pull', async ({}) => {
        ...
        let n = 1;
        for (const host of hosts) {
            let output = await cli.exec(`pmm-admin add mongodb --username=${MONGO_USERNAME} --password=${MONGO_PASSWORD} --metrics-mode=pull mongo_inst_${n++} ${host}`);
            await output.assertSuccess();
            await output.outContains('MongoDB Service added');
        }
    });
    ...
    test('run pmm-admin add mongodb based on running instances', async ({}) => {
        ...
        let n = 1;
        for (const host of hosts) {
            let output = await cli.exec(`pmm-admin add mongodb --username=${MONGO_USERNAME} --password=${MONGO_PASSWORD} mongo_inst_${n++} ${host}`);
            await output.assertSuccess();
            await output.outContains('MongoDB Service added');
        }
    });
    ...
    test('PMM-T160 User can\'t use both socket and address while using pmm-admin add mongodb', async ({}) => {
        ...
        for (const host of hosts) {
            ...
            let output = await cli.exec(`pmm-admin add mongodb --username=${MONGO_USERNAME} --password=${MONGO_PASSWORD} --socket=/tmp/mongodb-${port}.sock mongo_inst_${n++} ${host}`);
            await output.assertSuccess();
    ...
    test('run pmm-admin remove mongodb instance added based on running instances', async ({}) => {
        ...
        let n = 1;
        for (const host of hosts) {
            let output = await cli.exec(`pmm-admin remove mongodb mongo_inst_${n++}`);
            await output.assertSuccess();
            await output.outContains('Service removed.');
        }
    });
    ...
    test.skip(true, 'Skipping this test, because of random Failure');
        ...
        for (const host of hosts) {
            console.log(host);
            const port = host.split(':')[1];
            let output = await cli.exec(`pmm-admin add mongodb --username=${MONGO_USERNAME} --password=${MONGO_PASSWORD} --socket=/tmp/modb_${port}/mongodb-27017.sock mongo_inst_${n++}`);
            await output.assertSuccess();
            await output.outContains('MongoDB Service added');
        }
    });
    ...
    test('PMM-T964 run pmm-admin add mongodb with --agent-password flag', async ({}) => {
        ...
        for (const host of hosts) {
            const ip = host.split(':')[0];
            const port = host.split(':')[1];
            let output = await cli.exec(`pmm-admin add mongodb --username=${MONGO_USERNAME} --password=${MONGO_PASSWORD} --host=${ip} --agent-password=mypass --port=${port} --service-name=mongo_inst_${n++}`);
            await output.assertSuccess();
            await output.outContains('MongoDB Service added');
        }
    });
