...
const runContainerWithoutDataContainer = async (I) => {
  ...
  await I.verifyCommand(`docker run -v $HOME/srvNoData:/srv -d -e PMM_ENABLE_QAN_FOR_INTERNAL_POSTGRES=1 --restart always --publish 8081:8080 --name pmm-server-srv ${dockerVersion}`);
};
...
const runContainerWithPasswordVariable = async (I) => {
  ...
  await I.verifyCommand(`docker run -v $HOME/srvPassword:/srv -d -e GF_SECURITY_ADMIN_PASSWORD=newpass -e PMM_ENABLE_QAN_FOR_INTERNAL_POSTGRES=1 --restart always --publish 8082:8080 --name pmm-server-password ${dockerVersion}`);
};
...
const runContainerWithPasswordVariableUpgrade = async (I) => {
  ...
  await I.verifyCommand(`docker run -v $HOME/srvPasswordUpgrade:/srv -d -e GF_SECURITY_ADMIN_PASSWORD=newpass -e PMM_ENABLE_QAN_FOR_INTERNAL_POSTGRES=1 -e PMM_ENABLE_QAN_FOR_INTERNAL_POSTGRES=1 --restart always --publish 8089:8080 --name pmm-server-password-upgrade ${dockerVersion}`);
  I.wait(30);
...
const runContainerWithDataContainer = async (I) => {
  await I.verifyCommand(`docker run -e PMM_ENABLE_QAN_FOR_INTERNAL_POSTGRES=1 -v srvFolder:/srv -d --restart always --publish 8083:8080 --name pmm-server-empty-data-container ${dockerVersion}`);
};
