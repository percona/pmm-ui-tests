...
Data(instances).Scenario(
  ...
  + ' Verify that pmm-admin inventory add agent mongodb-exporter without --log-level flag adds MongoDB exporter with log-level=warn @inventory @exporters',
  async ({
    current, cliHelper,
  }) => {
    const {
...
Data(instances).Scenario(
  ...
  + ' Verify that pmm-admin inventory add agent qan-mongodb-profiler-agent without --log-level flag adds QAN MongoDB Profiler Agent with log-level=warn @inventory @exporters',
  async ({
