...
Scenario(
  ...
  async ({ I }) => {
    await I.say(await I.verifyCommand(`sudo pmm-admin add mongodb --port=${connection.port} --password=${connection.password} --username=${connection.username} --service-name=${mongodb_service_name} --enable-all-collectors`));
    let logs = '';
