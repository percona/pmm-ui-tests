...
Scenario(
  ...
  async ({ I, pmmInventoryPage }) => {
    ...
    // Find node ID
    const nodeId = (await I.verifyCommand(`docker exec ${connection.container_name} ls /usr/local/percona/pmm/tmp/agent_type_node_exporter/agent_id/`)).trim();
    ...
    await I.verifyCommand(`docker exec ${connection.container_name} rm /usr/local/percona/pmm/tmp/agent_type_node_exporter/agent_id/${nodeId}/webConfigPlaceholder`);
    const nodeFolder2 = await I.verifyCommand(`docker exec ${connection.container_name} ls /usr/local/percona/pmm/tmp/agent_type_node_exporter/agent_id/${nodeId}/`);
    ...
    const folderRestart = await I.verifyCommand(`docker exec ${connection.container_name} ls /usr/local/percona/pmm/tmp/agent_type_node_exporter/agent_id/${nodeId}/`);
    ...
    const folderRemoveNodeExporter = await I.verifyCommand(`docker exec ${connection.container_name} ls /usr/local/percona/pmm/tmp/agent_type_node_exporter/agent_id/${nodeId}/`);
