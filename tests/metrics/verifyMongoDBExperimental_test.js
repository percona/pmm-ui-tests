...
BeforeSuite(async ({ I, grafanaAPI, remoteInstancesHelper }) => {
  ...
  assert.ok(dockerCheck.includes('rs101') || dockerCheck.includes('psmdb'), 'rs101 or psmdb docker container should exist. please run pmm-framework with "mongo setup" options');
});
