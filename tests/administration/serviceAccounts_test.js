...
Scenario('PMM-T1883 Configuring pmm-agent to use service account @service-account', async ({
  ...
  await dashboardPage.verifyThereAreNoGraphsWithNA(1);
  await dashboardPage.verifyThereAreNoGraphsWithoutData(19);
  await I.verifyCommand(`sudo docker exec ps_pmm_8.0  pmm-admin add mysql --username=msandbox --password=msandbox --host=127.0.0.1  --port=8033 --service-name=${newServiceName}`);
  await I.wait(60);
