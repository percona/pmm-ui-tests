    ...
    async ({
      ...
      switch (serviceType) {
        ...
        case 'MONGODB_SERVICE':
          output = await I.verifyCommand(
            `pmm-admin add mongodb --node-id=${node_id} --username=pmm_mongodb --password=secret --pmm-agent-id=${pmm_agent_id} --port=27023 --host=${address} --agent-password=uitests --custom-labels="testing=upgrade" ${upgrade_service}`,
          );
          break;
        default:
      }
