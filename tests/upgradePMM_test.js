...
Scenario(
  'PMM-T289 Verify Whats New link is presented on Update Widget @pre-upgrade @pmm-upgrade',
  async ({ I, homePage }) => {
...
Scenario(
  'PMM-T288 Verify user can see Update widget before upgrade [critical] @pre-upgrade  @pmm-upgrade',
  async ({ I, homePage }) => {
    const versions = getVersions();
...
Scenario(
  'PMM-T391 Verify user is able to create and set custom home dashboard @pre-upgrade @pmm-upgrade',
...
Scenario(
  'Verify user can create Remote Instances before upgrade and they are in RUNNNING status @pre-upgrade @pmm-upgrade',
  async ({
...
Scenario(
  'PMM-T391 Verify that custom home dashboard stays as home dashboard after upgrade @post-upgrade @pmm-upgrade',
  async ({ I, grafanaAPI, dashboardPage }) => {
    I.amOnPage('');
    dashboardPage.waitForDashboardOpened();
    dashboardPage.verifyMetricsExistence(['Custom Panel']);
...
Scenario(
  'Verify Agents are RUNNING after Upgrade (API) [critical] @post-upgrade @pmm-upgrade',
  async ({ inventoryAPI }) => {
    for (const service of Object.values(inventoryAPI.services)) {
      if (service.service !== 'mongodb') await inventoryAPI.verifyServiceExistsAndHasRunningStatus(service, serviceNames[service.service]);
    }
...
Scenario(
  'Verify user can see Update widget [critical] @post-upgrade @pmm-upgrade',
  async ({ I, homePage }) => {
    I.amOnPage(homePage.url);
...
Scenario(
  'Verify user can see News Panel @post-upgrade @pmm-upgrade',
  async ({ I, homePage }) => {
    I.amOnPage(homePage.url);
...
Scenario(
  'PMM-T424 Verify PT Summary Panel is available after Upgrade @post-upgrade @pmm-upgrade',
...
Scenario(
  'Verify Agents are RUNNING after Upgrade (UI) [critical]  @post-upgrade @pmm-upgrade',
...
Scenario(
  'Verify QAN has specific filters for Remote Instances after Upgrade (UI) @post-upgrade @pmm-upgrade',
  async ({
...
Scenario(
  'Verify Metrics from custom queries for mysqld_exporter after upgrade (UI) @post-upgrade @pmm-upgrade',
  async ({ dashboardPage }) => {
...
Scenario(
  'PMM-T102 Verify Custom Prometheus Configuration File is still available at targets after Upgrade @post-upgrade @pmm-upgrade',
  async ({ I }) => {
