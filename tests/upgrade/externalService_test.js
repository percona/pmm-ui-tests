...
Data(remoteUpgradeInstances).Scenario(
  ...
  async ({ addInstanceAPI, current, remoteInstancesFixture }) => {
    ...
    await addInstanceAPI.apiAddInstance(remoteInstance.type, remoteInstance.service_upgrade_name);
  },
);
...
Data(remoteUpgradeInstances).Scenario(
  'PMM-T9995 - Verify Agents are Running and Metrics are being collected Post Upgrade (UI) [critical] @post-external-upgrade @post-client-upgrade',
