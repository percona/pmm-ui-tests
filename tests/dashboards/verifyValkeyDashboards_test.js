...
BeforeSuite(async () => {
  ...
  valkeyServiceName = response.service_name; // Ensure response shape matches.
});
...
Scenario(
  ...
  async ({ I, dashboardPage }) => {
    assert.ok(valkeyServiceName, 'Valkey service name was not resolved in BeforeSuite');
    const url = I.buildUrlWithParams(dashboardPage.ValkeyOverviewDashboard.url, {
      service_name: valkeyServiceName,
      from: 'now-5m',
    });
...
Scenario('PMM-T2087 - Open the Valkey Load dashboard and verify metrics @nightly @dashboards', async ({ I, dashboardPage }) => {
  assert.ok(valkeyServiceName, 'Valkey service name was not resolved in BeforeSuite');
  const url = I.buildUrlWithParams(dashboardPage.ValkeyLoadDashboard.url, { service_name: valkeyServiceName, from: 'now-5m' });
  I.amOnPage(url);
  dashboardPage.waitForDashboardOpened();
  await dashboardPage.expandEachDashboardRow();
