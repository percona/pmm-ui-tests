...
class MysqlAgentCli {
  async verifyMySqlAgentLogLevel(exporterType, dbDetails, logLevel = 'warn') {
    ...
    const actualLogLevel = await this.getLogLevel(agent_id, exporterType);

    I.say(`Actual log level is: ${actualLogLevel}`);
    I.assertEqual(actualLogLevel, this.getLogLevelResponse(logLevel), `Was expecting Mysql Exporter for service ${dbDetails.service_name} added again via inventory command and log level to have ${this.getLogLevelResponse(logLevel)} set, actual log level was: ${actualLogLevel}`);

    await I.verifyCommand(`docker exec ${dbDetails.container_name} pmm-admin inventory remove agent ${agent_id}`);
  }

  async getLogLevel(agentId, exporterType) {
    let output;
  ...
  getLogLevelResponse(logLevelFlag) {
    switch (logLevelFlag) {
      case 'warn':
