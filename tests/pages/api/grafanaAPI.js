...
module.exports = {
  ...
  async starDashboard(dashboardName) {
    const headers = { Authorization: `Basic ${await I.getAuth()}` };

    const resp = await I.sendPostRequest(`graph/api/user/stars/dashboard/${dashboardName}`, {}, headers);

    assert.ok(
      resp.status === 200,
      `Failed to star dashboard: '${dashboardName}' . Response message is ${resp.data.message}`,
    );
  },
